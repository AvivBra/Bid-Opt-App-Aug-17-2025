            ...
            if st.button(
                ...
                with st.spinner("Processing campaigns..."):
                    try:
                        ...
                        template_df = st.session_state.get("campaign_template_df")
                        data_dive_targets = st.session_state.get(
                            "data_dive_targets", {}
                        )
                        data_rova_df = st.session_state.get("data_rova_df")
                        selected = st.session_state.get("selected_campaigns", [])
                        ...
                        processor_logger = logging.getLogger(
                            "business.campaign_creator.processors.halloween_testing"
                        )
                        processor_logger.addHandler(handler)
                        processor_logger.setLevel(logging.INFO)

                        orchestrator_logger = logging.getLogger(
                            "business.campaign_creator.orchestrator"
                        )
                        ...
                        # Process campaigns
                        success, output_file, error = (
                            self.orchestrator.process_multiple_campaigns(
