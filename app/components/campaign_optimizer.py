"""Campaign Optimizer page - Based on Bid Optimizer layout."""
...
logging.basicConfig(
    level=logging.DEBUG,
    format="[%(asctime)s] %(levelname)s [CampaignOptimizer] %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
...
class CampaignOptimizerPage:
    ...
    def __init__(self):
        self.logger = logging.getLogger("CampaignOptimizer")
    ...
    def render(self):
        ...
        with col5:
            ...
            from business.portfolio_optimizations.factory import (
                get_portfolio_optimization_factory,
            )
                ...
                try:
                    ...
                    if uploaded_bulk.name.endswith(".csv"):
                        ...
                        if "Sponsored Products Campaigns" in bulk_data:
                            ...
                            self.logger.debug(
                                "Using 'Sponsored Products Campaigns' sheet"
                            )
                        else:
                            ...
                            if sheet_names:
                                ...
                                self.logger.warning(
                                    f"'Sponsored Products Campaigns' not found, using first sheet: {sheet_names[0]}"
                                )
                            else:
                    ...
                    self.logger.info(
                        f"File parsed successfully: {len(bulk_df)} rows, {len(bulk_df.columns)} columns"
                    )
                    self.logger.debug(
                        f"DataFrame columns: {list(bulk_df.columns)[:10]}..."
            ...
            if st.button(
                ...
                if files_ready and any_optimization_selected:
                    ...
                    self.logger.info(
                        f"Starting processing with optimizations: {selected_opts}"
                    )
                    self.logger.debug(
                        f"Files ready: {files_ready}, Any selected: {any_optimization_selected}"
                    )

                    st.success(
                        f"Processing {' and '.join(selected_opts)} optimization(s)..."
                    )
                    st.rerun()
                else:
                    if not files_ready:
            ...
            if st.session_state.get("portfolio_processing_status") == "processing":
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Retrieved DataFrame: {len(bulk_df)} rows, {len(bulk_df.columns)} columns"
                    )
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"All Excel sheets loaded: {list(all_sheets.keys())}"
                        )
                        ...
                        self.logger.warning(
                            f"Could not read all sheets: {str(e)}, using single DataFrame"
                        )
                    ...
                    from business.portfolio_optimizations.results_manager import (
                        PortfolioOptimizationResultsManager,
                    )
                    ...
                    if not selected_opts:
                        ...
                        st.warning(
                            "No optimizations selected. Please select at least one optimization."
                        )
                        st.session_state.portfolio_processing_status = "error"
                        st.rerun()
                        return
                    ...
                    self.logger.info(
                        f"Processing {len(selected_opts)} selected optimizations: {selected_opts}"
                    )
                    ...
                    for i, opt_name in enumerate(selected_opts):
                        ...
                        try:
                            ...
                            if orchestrator is None:
                                self.logger.error(
                                    f"Failed to create orchestrator for {opt_name}"
                                )
                                st.error(
                                    f"Failed to initialize {display_name} optimization"
                                )
                                continue

                            self.logger.debug(
                                f"{display_name} orchestrator instantiated"
                            )

                            # Run optimization with standardized interface
                            self.logger.debug(
                                f"Calling run with data: {list(all_sheets.keys())}"
                            )
                            processed_df, processing_details = orchestrator.run(
                                all_sheets, combined_mode=True
                            )

                            self.logger.info(
                                f"{display_name} result: DataFrame is not None: {processed_df is not None}"
                            )
                            if processed_df is not None:
                                self.logger.debug(
                                    f"Processed DataFrame: {len(processed_df)} rows"
                                )
                            self.logger.debug(
                                f"Processing details: {processing_details}"
                            )

                            # FIX: Determine result type based on specific optimization name
                            # campaigns_without_portfolios should update campaigns, not portfolios!
                            if opt_name == "empty_portfolios":
                                result_type = "portfolios"
                            else:
                                # campaigns_without_portfolios and any future optimizations
                                result_type = "campaigns"

                            self.logger.info(
                                f"Setting result_type='{result_type}' for {opt_name}"
                            )
                            ...
                            if processed_df is not None:
                                ...
                                self.logger.error(
                                    f"{display_name} returned None DataFrame"
                                )
                            ...
                            self.logger.debug(
                                f"{display_name} traceback: {traceback.format_exc()}"
                            )
                    ...
                    if results_manager.has_any_successful_results():
                        self.logger.info(
                            "Creating combined output file from multiple optimizations"
                        )
                        try:
                            combined_file, summary_details = (
                                results_manager.create_combined_output(all_sheets)
                            )

                            if combined_file:
                                st.session_state.portfolio_working_file = (
                                    combined_file.getvalue()
                                )
                                st.session_state.portfolio_output_generated = True

                                # Log summary
                                successful_count = summary_details.get(
                                    "successful_optimizations", 0
                                )
                                total_count = summary_details.get(
                                    "total_optimizations", 0
                                )
                                campaigns_updated = summary_details.get(
                                    "campaigns_updated", 0
                                )
                                portfolios_updated = summary_details.get(
                                    "portfolios_updated", 0
                                )

                                self.logger.info(
                                    f"Combined output file created successfully: {successful_count}/{total_count} optimizations"
                                )
                                self.logger.info(
                                    f"Updates: {campaigns_updated} campaigns, {portfolios_updated} portfolios"
                                )
                                ...
                                error_msg = summary_details.get(
                                    "error", "Unknown error creating combined file"
                                )
                                self.logger.error(
                                    f"Failed to create combined output file: {error_msg}"
                                )
                                st.error(
                                    f"Failed to create combined output file: {error_msg}"
                                )
                    ...
                    has_working_file = (
                        st.session_state.get("portfolio_working_file") is not None
                    )
                    self.logger.info(
                        f"Final check - Working file in session state: {has_working_file}"
                    )
                ...
                error_msg = st.session_state.get(
                    "portfolio_processing_error", "Unknown error"
                )
                ...
                if st.button(
                    "Try Again", key="btn_retry_portfolio", use_container_width=True
                ):
