...
def render_template_card() -> None:
    ...
    if template_file:
        render_file_card(
            title="TEMPLATE",
            file_name=template_file.name if hasattr(template_file, 'name') else "Template.xlsx",
            status="uploaded",
...
def render_bulk_card() -> None:
    ...
    if bulk_file:
        ...
        render_file_card(
            ...
            file_name=bulk_file.name if hasattr(bulk_file, 'name') else f"Bulk_{days}.xlsx",
            status="uploaded",
...
def get_bulk_details(bulk_type: Optional[str]) -> Dict[str, Any]:
    ...
    details = {
        'Total Rows': f"{len(bulk_df):,}",
        'Columns': len(bulk_df.columns)
    }
...
def clear_file_cards() -> None:
    ...
    keys_to_clear = [
        'template_file', 'template_df',
        'bulk_7_file', 'bulk_7_df',
        'bulk_30_file', 'bulk_30_df',
        'bulk_60_file', 'bulk_60_df'
    ]
