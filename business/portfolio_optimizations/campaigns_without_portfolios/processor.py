...
class CampaignsWithoutPortfoliosProcessor:
    ...
    def process(
        self, cleaned_data: Dict[str, pd.DataFrame], combined_mode: bool = False
        ...
        campaigns_without_portfolio = campaigns_filtered[
            campaigns_without_portfolio_mask
        ]
        ...
        if len(campaigns_without_portfolio) > 0:
            ...
            for idx, row in campaigns_without_portfolio.iterrows():
                ...
                mask = (campaigns_full["Entity"] == TARGET_ENTITY) & (
                ...
                if full_indices:
                    ...
                    # Convert Portfolio ID to string to avoid type issues
                    campaigns_full.at[full_idx, "Portfolio ID"] = str(
                        TARGET_PORTFOLIO_ID
                    )
                    ...
                    self.logger.debug(
                        f"Updated campaign at index {full_idx}: "
                        f"Campaign ID={row['Campaign ID']}, "
                        f"New Portfolio ID={TARGET_PORTFOLIO_ID}"
                    )

            self.logger.info(
                f"Updated {len(self.updated_indices)} campaigns with portfolio ID {TARGET_PORTFOLIO_ID}"
            )
        ...
        # Log summary
        self.logger.info(
            f"Processing complete: {self.processing_stats['campaigns_updated']}/{self.processing_stats['total_campaigns']} "
            f"campaigns updated"
        )
    ...
    def merge_with_empty_portfolios_results(
        self, campaigns_df: pd.DataFrame, empty_portfolios_indices: List[int]
    ) -> pd.DataFrame:
        """
